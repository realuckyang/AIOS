// 工具分发钩子(FUSE 式):机制在内核,工具是什么由 userland 声明。
// userland/tools.json 声明 [{name, description, parameters, exec}, {type: "web_search"}, ...]。
...
export function loadRegistry(config) {
  const file = config.tools ? path.resolve(ROOT, config.tools) : path.join(ROOT, 'userland', 'tools.json');
...
export function dispatch(execPath, argumentsJson, { cwd, timeoutMs, signal } = {}) {
  return new Promise((resolve) => {
    ...
    child.on('close', (code) => {
      ...
      try {
        JSON.parse(stdout);
        resolve(stdout.trim()); // 合法 JSON:exec 自己决定输出形状,原样透传
      } catch {
        resolve(JSON.stringify({ exit_code: killed ? -1 : (code ?? -1), stdout, stderr }));
      }
    });
  });
}
