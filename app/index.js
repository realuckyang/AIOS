// App 进程入口:装配框架、挂载应用、静态托管构建产物。
// 构建:cd app && npm install && npm run build
// 开发:cd app && npm run dev → http://127.0.0.1:5173（/api 代理到 App）
import http from 'node:http';
import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import crypto from 'node:crypto';
import { handleApi } from './main/server/api/index.js';
import { ensureVarDir } from './main/server/db/client.js';
import { completeRestartRequests } from './main/server/repository/restarts.js';
import { configure as configureEvents } from './main/server/events.js';
import { seedDefaults, getConfig } from './main/server/config.js';
...
// 边界复核:apps/ 下不许出现通往框架库/仓库的 import。约定靠不住,所以每次启动查一遍。
try {
  const { execFileSync } = await import('node:child_process');
  execFileSync(process.execPath, [path.join(ROOT, '..', 'bin', 'hooks', 'check-boundaries')], { stdio: 'inherit' });
} catch {
  console.warn('[app] 边界检查未通过或不可执行 —— 继续启动,但请尽快处理');
}

ensureVarDir();          // settings 表在库里,所以配置要等库就绪之后才合并
seedDefaults();          // 把默认值写进库 —— 库从此是完整的当前状态
...
http.createServer(async (req, res) => {
  ...
  if (!fs.existsSync(file)) {
    ...
    return res.end('UI 尚未构建: cd app && npm run build\n');
  }
