...
import type { UsageThread, UsageOverview, UsageTrendPoint } from '../../../main/ui/types';
...
export default function UsageApp() {
  ...
  const [threads, setThreads] = useState<UsageThread[]>([]);
  ...
  useEffect(() => {
    ...
    api.getUsageThreads().then((c) => { if (alive) setThreads(c.threads); }).catch((e: Error) => { if (alive) setError(e.message); });
    return () => { alive = false; };
  }, []);
          ...
          <>
            ...
            <div className="usage-panel">
              ...
              <div className="usage-table">
                ...
                {/* 对话与任务同列:压缩这类非用户发起的消耗从此看得见 */}
                {threads.map((c) => (
                  <div key={c.id} className="usage-tr">
                    <span className="usage-tc-title" title={c.title}>
                      {c.kind === 'task' && <span className="usage-kind">{c.app || '任务'}</span>}
                      {c.title || c.id}
                    </span>
                ...
                {!threads.length && <div className="usage-empty">还没有用量记录</div>}
              </div>
            </div>
          </>
        )}
