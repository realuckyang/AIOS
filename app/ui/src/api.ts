...
import type { AiosConfig, AppHealth, ChatMeta, Memory, RestartRequest, Row, SkillDetail, SkillSummary, Todo, ToolDetail, ToolSummary } from './types';
...
export const getConfig = () => request<AiosConfig>('GET', '/config');
export const updateConfig = (config: AiosConfig) =>
  request<{ config: AiosConfig; restartRequired: boolean }>('PATCH', '/config', config);
