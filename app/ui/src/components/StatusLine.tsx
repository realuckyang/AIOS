...
import type { AiosConfig, ChatMeta, Row } from '../types';
...
export const StatusLine = memo(function StatusLine({ items, meta }: { items: Row[]; meta: ChatMeta | null }) {
  ...
  useEffect(() => {
    api.getConfig().then(setConfig).catch(() => { /* 拿不到配置就少显示几段 */ });
  }, []);

  const last = [...items].reverse().find((row) => row.usage)?.usage;
  if (!meta || !last) return null;

  const window = config?.contextWindowTokens ?? 0;
  ...
  const priceIn = config?.priceInputPerMTokens ?? 0;
  const priceOut = config?.priceOutputPerMTokens ?? 0;
  const priceCached = (config?.priceCachedPerMTokens ?? 0) > 0 ? config!.priceCachedPerMTokens : priceIn;
