...
import { runInstant } from './task.js';
...
async function summarize({ material, strict, prompt, timeoutMs, kernelPort }) {
  const base = String(prompt || '').trim() || DEFAULT_COMPACTION_PROMPT;
  const instructions = strict ? `${base}\n\n再说一次:只输出摘要正文本身。` : base;
  // 走 task 而不是直连 /api/complete:摘要这次请求同样开线程、落消息、记账。
  // 老版这一笔只写进 compactions.tokens,状态行和用量应用都看不见 —— 那是漏账。
  return runInstant({
    app: 'main', title: '上下文折叠', prompt: material,
    instructions, timeoutMs, kernelPort,
  });
}
 ...
 */
const foldStreak = new Map(); // threadId → { count, endSeq, warned }

/**
 * 水位维护。返回是否产生了新的 compaction(调用方据此决定要不要重新组装)。
 * 任何失败都只是「这次不折」,不该让 run 挂掉。
 */
export async function maintainContext(threadId, { kernelPort, appApiBase, config }) {
  ...
  if (tokens < Math.round(window * config.compactFoldRatio)) {
    foldStreak.delete(threadId);
    return false;
  }

  const start = meta.context_start || 0;
  const folds = compactions.allCompactions(threadId).filter((row) => row.start_seq > start);
  const floor = Math.max(start, folds.length ? Number(folds.at(-1).end_seq) : 0);
  ...
  const streak = foldStreak.get(threadId) || { count: 0, endSeq: 0, warned: false };
  const blocked = streak.count >= 2 && endSeq <= streak.endSeq;

  if (keepAt < (forced ? 1 : 3) || blocked) {
    if (!streak.warned) {
      console.warn(`[app] ${threadId} 水位 ${tokens}/${window} 但已无可折叠素材,暂停折叠`);
      foldStreak.set(threadId, { ...streak, warned: true });
    }
    return false;
  }
  ...
  compactions.insertCompaction(threadId, { startSeq, endSeq, summary, kind, tokens: tokensUsed });
  foldStreak.set(threadId, { count: streak.count + 1, endSeq, warned: false });
  console.log(`[app] ${threadId} 折叠 seq ${startSeq}–${endSeq}(${kind},水位 ${tokens}/${window})`);
  return true;
}
