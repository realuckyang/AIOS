...
// 水位：分子是最近一次请求的 input+output，分母是配置的上下文窗口。
// agent 靠这一段判断该不该压缩，所以绝对值和百分比都要给。
function watermark(usage, window) {
  if (!usage || !(window > 0)) return null;
  const level = (usage.input_tokens ?? 0) + (usage.output_tokens ?? 0);
  const pct = Math.round((level / window) * 100);
  return `上下文水位: ${compact(level)}/${compact(window)} (${pct}%)`;
}

function stateItem(runId, state, usage, config) {
  const latest = usage ?? state.lastUsage ?? null;
  const tokens = latest
    ? `input=${latest.input_tokens ?? '?'} output=${latest.output_tokens ?? '?'}`
    : '无(尚无模型请求)';
  const text = [
    `[kernel 状态行] run=${runId}`,
    state.threadId ? `thread=${state.threadId}` : null,
    `run 起始 seq=${state.latestSeq ?? 0}`,
    `context_start=${state.contextStart ?? 0}`,
    `上次请求 token 用量: ${tokens}`,
    watermark(latest, config?.contextWindowTokens),
...
export async function runLoop({ runId, input, state = {}, config, instructions, signal, emit }) {
  ...
  while (true) {
    ...
    const { items, usage } = await llm.request({
      ...
      model: config.model,
      instructions,
      input: [...input, ...generated, stateItem(runId, state, lastUsage, config)],
